...
func endsWithCSSKeyword(b string, kw string) bool {
	i := len(b) - len(kw)
	if i < 0 {
		// Too short.
		return false
	...
	if i != 0 {
		r, _ := utf8.DecodeLastRuneInString(b[:i])
		if isCSSNmchar(r) {
			// Too long.
			return false
		}
...
func decodeCSS(s string) string {
...
func skipCSSSpace(c string) string {
	if len(c) == 0 {
		return c
	}
	// wc ::= #x9 | #xA | #xC | #xD | #x20
...
func cssValueFilter(args ...interface{}) string {
	...
	b, id := decodeCSS(s), make([]byte, 0, 64)
	...
	for i, c := range b {
		...
		default:
			if c < utf8.RuneSelf && isCSSNmchar(c) {
				id = append(id, byte(c))
			}
		}
	}
